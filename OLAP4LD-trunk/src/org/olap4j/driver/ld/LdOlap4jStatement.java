 ...
 * @since May 24, 2007
 */
class LdOlap4jStatement implements OlapStatement {
	 ...
	 * {@link LdOlap4jStatement}.
	 */
	LdOlap4jCellSet openCellSet;
	 ...
	 *            Connection
	 */
	LdOlap4jStatement(LdOlap4jConnection olap4jConnection) {
	...
	public CellSet executeOlapQuery(String mdx) throws OlapException {
		...
		LdOlap4jUtil._log.info("Parse MDX: " + mdx);
	...
	public CellSet executeOlapQuery(SelectNode selectNode) throws OlapException {
		...
		synchronized (this) {
			if (openCellSet != null) {
				final LdOlap4jCellSet cs = openCellSet;
				openCellSet = null;
				try {
					cs.close();
		...
		// Axes
		List<LdOlap4jCellSetAxis> axisList = new ArrayList<LdOlap4jCellSetAxis>();
		...
		LdOlap4jCellSetAxisMetaData filterAxisMetaData = new LdOlap4jCellSetAxisMetaData(
	 ...
	 * @return
	 */
	private List<Level> createLevelList(List<Position> axisPositions) {
		/*
		 * The list of hierarchies used in an axis, the dimensionality in terms
		 * of hierarchies
		 */
	...
	static class SimpleNonEmptyResult implements CellSet {
		...
		SimpleNonEmptyResult(CellSet result, SelectNode selectNode)
			...
			LdOlap4jCellSetAxis columnAxis = (LdOlap4jCellSetAxis) mutableAxesList
					.get(0);
			...
			LdOlap4jCellSetAxis rowAxis = (LdOlap4jCellSetAxis) mutableAxesList
					.get(1);
			...
			if (columnAxisNode.isNonEmpty()) {
				...
				mutableAxesList.set(0, new LdOlap4jCellSetAxis(
						(LdOlap4jCellSet) result, mutableAxesList.get(0)
			...
			if (rowAxisNode.isNonEmpty()) {
				...
				mutableAxesList.set(1, new LdOlap4jCellSetAxis(
						(LdOlap4jCellSet) result, mutableAxesList.get(1)
	 ...
	 */
	static class NonEmptyResult extends LdOlap4jCellSet {
		...
		NonEmptyResult(CellSet result, SelectNode query, int axis)
			...
			// copy of all axes
			// super(query, result.getAxes().clone());
			super((LdOlap4jStatement) result.getStatement());
			// copy
			...
			// list
			this.mutableAxesList.set(axis, new LdOlap4jCellSetAxis(
					(LdOlap4jCellSet) result, mutableAxesList.get(axis)
