 ...
 * to the olap4j driver for LD.
 * 
 * In order to test LdOlap4j we have copied over the XmlaTester. The ConnectionTest (or any other test)
 * only needs to be run from OLAP4LD so that the test.properties are read.
 * 
 * However, the tests seem to be very Mondrian or Xmla specific. We are not sure whether those
 * tests are directly of use for olap4ld; however, they are useful for sure if one wants to create
 * own tests on olap4ld, e.g., on reference RDF Data Cube data. One could transform the 
 * testing data of olap4j for use by olap4ld, but that probably would be some work (although,
 * maybe that even would be a good evaluation).
 *
...
public class LdOlap4jTester implements TestContext.Tester {
     ...
     */
    public LdOlap4jTester(TestContext testContext)
