...
Changelog:
* 2013-07-02: Added strip-space for Sesame SPARQL/XML Output
...
<xsl:stylesheet version="1.0"
	...
	<xsl:template name="vb-result">
		...
		<xsl:for-each select="res:head/res:variable">?<xsl:value-of select="@name" /><xsl:text> </xsl:text></xsl:for-each>
		<!-- do not forget the . and newline -->
		<xsl:text>.&#10;</xsl:text>
	...
	<xsl:template match="res:uri">
			<xsl:variable name="uri" select="text()" /><xsl:text disable-output-escaping="yes">&lt;</xsl:text><xsl:value-of select="$uri" /><xsl:text disable-output-escaping="yes">&gt;</xsl:text><xsl:text> </xsl:text>
	</xsl:template>
